...
st.text(f"Data calculului (Local): {acum_local.strftime('%d-%m-%Y')} | Ora actuala: {acum_local.strftime('%H:%M:%S')}")
st.text(f"Coordonate fixe: {LATITUDINE} N, {LONGITUDINE} E")
st.text("=" * 60)
...
for nume, corp_id in corpuri.items():
    ...
    for nume_ev, jd_ev in ore_orizont.items():
        if jd_ev:
            ...
            st.text(f"    - {nume_ev:<22} : {dt_ev.strftime('%H:%M:%S')}")
            if corp_id == swe.SUN:
                date_orizont_soare[nume_ev] = dt_ev
        else:
            st.text(f"    - {nume_ev:<22} : Date indisponibile")
    ...
    try:
        ...
        if corp_id == swe.MOON:
            ...
            st.text(f"    - Manzil al-Qamar (Luna) : Stația {m_luna['numar']:02d}/28 - {m_luna['nume_arab']} ({m_luna['traducere']}) | Poziție: {m_luna['progres_text']}")
        # ------------------------------------------------------
...
if dt_r and dt_a:
    ...
    st.text("\n[ORE PLANETARE DE ZI]")
    for numar, planeta, start, end in ore_zi:
        st.text(f"  Ora {numar:02d} ({planeta:<7}) : {start.strftime('%H:%M:%S')} - {end.strftime('%H:%M:%S')}")
...
try:
    ...
    for eticheta, pozitie_text in tabel_case.items():
        st.text(f"  - {eticheta:<15} : {pozitie_text}")
except Exception as e:
...
for nume, corp_id in puncte_fictive.items():
    try:
        ...
        if "Nod Nord" in nume:
            ...
            st.text(f"  - Nod Sud ({tip_nod:<4}) : {format_pozitie_astrologica(lon_sud):<25} | Casa: {numar_casa_sud:02d} | ({p['miscare']})")
...
for nume_afisat, nume_se in stele_fixe.items():
    try:
        ...
        st.text(f"  - {nume_afisat:<19} : {s['pozitie_text']:<25} | Casa: {numar_casa:02d}")
        
        # Le salvăm în dicționarul general ÎNAINTE ca aspectele să fie calculate
        coordonate_totale[nume_afisat] = s['lon_pura']
    except Exception as e:
        st.text(f"  - {nume_afisat:<19} : Eroare la calcul (Nume invalid în SE?): {e}")
...
# =====================================================================
st.text("\n" + "=" * 60)
st.text("[EVALUAREA DINAMICĂ A FORȚEI PLANETARE (SCORURI DE TRANZIT)]")
...
st.text("\n" + "=" * 60)
st.text("[SCORUL COSMIC GLOBAL AL MOMENTULUI]")
st.text(f"  - Indice de eficiență planetară totală: {scor_cosmic_global:.1f}%")
...
# =====================================================================
st.text("\n" + "=" * 60)
st.text("[PANOU FILOSOFIC / PUNCTE ARABE MAJORE]")
...
st.text(f"  - Tipul Sectei (Harta)     : {'DIURNĂ (Zi)' if este_harta_diurna else 'NOCTURNĂ (Noapte)'}")
...
# =====================================================================
st.text("\n" + "=" * 60)
st.text("[EXPORT DATE - JSON EXTINS]")
...
try:
    ...
    st.text(f"  - Succes! Fișierul 'date_dashboard.json' extins a fost generat.")
except Exception as e:
    st.text(f"  - Eroare la scrierea fișierului JSON: {e}")

# Închiderea obligatorie a resurselor Swiss Ephemeris

st.text("✅ Calcul complet - pagină încărcată cu succes")
swe.close()
